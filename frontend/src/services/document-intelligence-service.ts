...
export const documentIntelligenceService = {
  ...
  uploadAndAnalyze: async (file: File) => {
    ...
    return documentIntelligenceService.analyze(uploaded.id);
  },
  uploadManyAndAnalyze: async (files: File[]) => {
    const results: DocumentAnalysis[] = [];
    for (const file of files) results.push(await documentIntelligenceService.uploadAndAnalyze(file));
    return results;
  },

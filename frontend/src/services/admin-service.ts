...
export const adminService = {
  ...
  async updateUser(id: number, payload: Partial<Pick<AdminUser, "email" | "full_name" | "role" | "active">> & { password?: string }) { const { data } = await apiClient.put<{ success: boolean; user: AdminUser }>(`/api/users/${id}`, payload); return data.user; },
  ...
  async listProducts() { const { data } = await apiClient.get<{ success: boolean; products: AdminProduct[] }>("/api/catalog/products", { params: { active: "false" } }); return data.products; },
  async deleteProduct(id: number) { await apiClient.delete(`/api/admin/products/${id}`); },
  async listOrders() { const { data } = await apiClient.get<{ success: boolean; orders: AdminOrder[] }>("/api/orders", { params: { limit: 100 } }); return data.orders; },
  async deleteOrder(id: number) { await apiClient.delete(`/api/admin/orders/${id}`); },
  async listImports() { const { data } = await apiClient.get<{ success: boolean; sessions: AdminImport[] }>("/api/imports"); return data.sessions; },

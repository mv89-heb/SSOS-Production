  ...
  return (
    <div className="space-y-8 pb-8">
      <section>
        ...
        <p className="page-subtitle">כאן רואים הזמנות שצריך לחזור אליהן. התזכורת מחושבת לפי חלונות ההזמנה שהוגדרו לספק.</p>
      </section>
      ...
      <Card className="glass-card border-indigo-100 dark:border-indigo-900/50">
        <CardContent className="flex flex-col gap-2 p-4 text-sm text-slate-600 dark:text-slate-300 md:flex-row md:items-center md:gap-4">
          ...
          <p><span className="font-extrabold text-slate-900 dark:text-white">איך זה עובד?</span> מפעילים תזכורת מתוך הזמנה פתוחה. המערכת לוקחת את כללי ההזמנה של הספק ומחשבת את מועד המעקב הבא. כשהמועד מגיע, ההזמנה עוברת ל״דורשת טיפול״. אפשר לפתוח את ההזמנה, לסמן שטיפלת או לדחות את התזכורת.</p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        ...
        <CardContent className="pt-3">
          {isLoading ? <div className="space-y-3 py-6">{[1,2,3].map((item) => <div key={item} className="h-20 animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800" />)}</div>
            ...
            : ordered.length === 0 ? <div className="py-14 text-center"><Bell className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700 dark:text-slate-200">אין כרגע תזכורות פעילות</p><p className="mt-1 text-xs text-slate-400">פתח הזמנה שעדיין בטיפול והפעל בה תזכורת. אם אין אפשרות להפעיל, צריך להגדיר לספק חלונות הזמנה.</p></div>
            : <div className="divide-y divide-slate-100 dark:divide-slate-800">{ordered.map(({ order, next_reminder_at }) => {
              ...
              return <div key={order.id} className="flex flex-col gap-4 px-2 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-800/60 md:flex-row md:items-center md:justify-between">
                <Link href={`/dashboard/orders/${order.id}`} className="flex min-w-0 items-center gap-3 md:flex-1">
                  ...
                  <div className="min-w-0"><div className="truncate text-sm font-extrabold text-slate-900 dark:text-white">{order.order_number}</div><div className="mt-0.5 truncate text-xs text-slate-400">{order.supplier_name}</div><div className="mt-1 text-xs text-slate-400">{isDue ? "הגיע מועד המעקב" : `מועד: ${formatDate(next_reminder_at)}`}</div></div>
                </Link>

...
import { useOpenReminders } from "@/hooks/use-reminders";
    ...
    <div className="space-y-8 pb-8">
      ...
      <Card className="glass-card">
        ...
        <CardContent className="pt-3">
          {isLoading ? <div className="space-y-3 py-6">{[1,2,3].map((item) => <div key={item} className="h-16 animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800" />)}</div>
            ...
            : ordered.length === 0 ? <div className="py-14 text-center"><Bell className="mx-auto h-10 w-10 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700 dark:text-slate-200">אין כרגע תזכורות פעילות</p><p className="mt-1 text-xs text-slate-400">ניתן להפעיל תזכורת מתוך פרטי הזמנה כאשר לספק מוגדרים כללי הזמנה.</p></div>
            : <div className="divide-y divide-slate-100 dark:divide-slate-800">{ordered.map(({ order, next_reminder_at }) => {
              ...
              return <Link key={order.id} href={`/dashboard/orders/${order.id}`} className="flex items-center justify-between gap-4 rounded-xl px-2 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
                <div className="flex min-w-0 items-center gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${isDue ? "bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300" : "bg-indigo-50 text-indigo-600 dark:bg-indigo-950/50 dark:text-indigo-300"}`}><ShoppingBag className="h-5 w-5" /></div><div className="min-w-0"><div className="truncate text-sm font-extrabold text-slate-900 dark:text-white">{order.order_number}</div><div className="mt-0.5 truncate text-xs text-slate-400">{order.supplier_name}</div></div></div>
                <div className="shrink-0 text-left"><div className={`text-xs font-extrabold ${isDue ? "text-amber-700 dark:text-amber-300" : "text-slate-600 dark:text-slate-300"}`}>{isDue ? "דורש טיפול" : "תזכורת הבאה"}</div><div className="mt-1 text-xs text-slate-400">{formatDate(next_reminder_at)}</div></div>
              </Link>;
            })}</div>}
        </CardContent>
      </Card>

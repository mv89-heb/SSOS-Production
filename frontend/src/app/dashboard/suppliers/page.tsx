...
import { useSuppliers, useCreateSupplier } from "@/hooks/use-catalog";
...
export default function SuppliersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isError && <ErrorState description="Could not load suppliers." onRetry={() => refetch()} />}

      {!isLoading && !isError && filtered.length === 0 && (
        <EmptyState
          icon={Users}
          title={suppliers && suppliers.length > 0 ? "No suppliers match your search" : "No suppliers yet"}
          description={canManage ? "Add a supplier before creating products against it." : undefined}
          actionLabel={canManage && (!suppliers || suppliers.length === 0) ? "New Supplier" : undefined}

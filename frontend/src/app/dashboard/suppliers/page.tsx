...
import { Check, Package, Pencil, Phone, Plus, Search, ShoppingCart, Users, X } from "lucide-react";
...
export default function SuppliersPage() {
  ...
  const productCountBySupplier = useMemo(() => {
    ...
    (allProducts ?? []).forEach((product) => counts.set(product.supplier_id, (counts.get(product.supplier_id) ?? 0) + 1));
    return counts;
  }, [allProducts]);

  const filtered = useMemo(() => {
    const query = search.trim().toLowerCase();
    return (suppliers ?? []).filter((supplier) => !query || [supplier.name, supplier.contact_name, supplier.email, supplier.phone, supplier.customer_number, supplier.order_days, supplier.delivery_days].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)));
  }, [suppliers, search]);
  ...
  const saveEdit = () => {
    ...
    updateSupplier.mutate({ [editing.field]: value || undefined }, { onSuccess: () => setEditing(null) });
  };

  const cell = (supplier: Supplier, field: EditableField, display: React.ReactNode, className = "") => {
    const active = editing?.id === supplier.id && editing.field === field;
    if (!canManage) return <span className={className}>{display}</span>;
    if (!active) return <button type="button" onClick={() => startEdit(supplier, field)} title="עריכה ישירה" className={`group inline-flex min-h-9 w-full items-center justify-between gap-2 rounded-lg px-2 text-right transition hover:bg-indigo-50 dark:hover:bg-indigo-950/40 ${className}`}><span>{display}</span><Pencil className="h-3.5 w-3.5 shrink-0 text-indigo-400 opacity-0 transition group-hover:opacity-100" /></button>;
  ...
  const handleSubmit = () => {
    ...
    createSupplier.mutate({ name: form.name.trim(), contact_name: form.contact_name.trim() || undefined, email: form.email.trim() || undefined, phone: form.phone.trim() || undefined, phone2: form.phone2.trim() || undefined, customer_number: form.customer_number.trim() || undefined, delivery_days: form.delivery_days.trim() || undefined, order_days: form.order_days.trim() || undefined }, { onSuccess: () => setModalOpen(false) });
  };
  ...
  return <div className="space-y-6 pb-8" dir="rtl">
    ...
    {!isLoading && !isError && filtered.length > 0 && <div className="overflow-x-auto rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><Table><TableHead><tr><TableHeaderCell>ספק</TableHeaderCell><TableHeaderCell>איש קשר</TableHeaderCell><TableHeaderCell>טלפונים</TableHeaderCell><TableHeaderCell>מס' לקוח</TableHeaderCell><TableHeaderCell>ימי הזמנות</TableHeaderCell><TableHeaderCell>ימי אספקה</TableHeaderCell><TableHeaderCell>מוצרים</TableHeaderCell><TableHeaderCell>סטטוס</TableHeaderCell><TableHeaderCell>פעולה</TableHeaderCell></tr></TableHead><TableBody>{filtered.map((supplier) => { const productCount = productCountBySupplier.get(supplier.id) ?? 0; return <TableRow key={supplier.id}>
      <TableCell>{cell(supplier, "name", <Link href={`/dashboard/suppliers/${supplier.id}`} className="font-extrabold text-slate-900 hover:text-indigo-700 dark:text-white">{supplier.name}</Link>)}</TableCell>
      <TableCell>{cell(supplier, "contact_name", supplier.contact_name || "—")}</TableCell>
      <TableCell><div className="space-y-1 text-xs">{cell(supplier, "phone", supplier.phone ? <a href={`tel:${supplier.phone}`} className="flex items-center gap-1.5 font-mono"><Phone size={12} />{supplier.phone}</a> : "—")} {supplier.phone2 && cell(supplier, "phone2", <span className="flex items-center gap-1.5 font-mono text-slate-400"><Phone size={12} />{supplier.phone2}</span>)}</div></TableCell>
      ...
      <TableCell><Link href={`/dashboard/orders/new?supplier_id=${supplier.id}`} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-2.5 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-950/60 dark:text-indigo-300"><ShoppingCart size={14} />הזמנה</Link></TableCell>

...
export default function DocumentIntelligencePage() {
  ...
  async function apply() {
    ...
    const lines = items.map((item, index) => {
      ...
      const autoProduct = product?.decision === "AUTO_MATCH" ? product.product_id : undefined;
      const autoSupplier = supplier?.decision === "AUTO_MATCH" ? supplier.supplier_id : undefined;
      return { product_id: map.product || autoProduct, supplier_id: map.supplier || autoSupplier, price: Number(item.unit_price), currency: data?.currency || "ILS", unit: item.unit, package_quantity: item.package_quantity, update_price: Boolean(map.update), match_method: product?.method || "MANUAL_REVIEW", match_confidence: product?.confidence };
    });
    if (lines.some((line) => !line.product_id || !line.supplier_id || !Number.isFinite(line.price) || line.price <= 0)) { setError("יש לאשר התאמת מוצר וספק לכל שורה שלא זוהתה בוודאות גבוהה, ולוודא שמחיר השורה תקין."); return; }
  ...
  const defaultProduct = (item: ExtractedItem, index: number) => mappings[index]?.product || (item.product_matching?.decision === "AUTO_MATCH" ? item.product_matching.best_match?.product_id || "" : "");

...
export default function DocumentIntelligencePage() {
  ...
  const appliedIndexes = useMemo(() => new Set<number>((Array.isArray(data?.applied_line_indexes) ? data.applied_line_indexes : []).map(Number)), [data]);
  ...
  async function apply() {
    ...
    const pending = lines.filter((line) => !appliedIndexes.has(line.line_index));
    const ready = pending.filter((line) => line.product_id && line.supplier_id);
    if (!ready.length) { setError("אין כרגע שורות חדשות שניתן ליישם. בחר מוצר וספק בשורות שסומנו לבדיקה."); return; }

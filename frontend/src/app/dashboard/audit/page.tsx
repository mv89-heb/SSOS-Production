...
export default function AuditPage() {
  ...
  if (!canView) {
    return (
      <ErrorState
        title="No permission"
        description="Audit trail is only visible to managers and admins."
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {isError && <ErrorState description="Could not load audit logs." onRetry={() => refetch()} />}

      {!isLoading && !isError && (!logs || logs.length === 0) && (
        <EmptyState icon={ClipboardList} title="No audit events yet" description="Actions taken across the system will show up here." />
      )}
      ...
      {!isLoading && !isError && logs && logs.length > 0 && (
        <Table>
          ...
          <TableBody>
            {logs.map((log) => (
              <TableRow key={log.id}>
                <TableCell className="text-slate-900">{log.user_full_name || log.user_email || "System"}</TableCell>
                <TableCell>

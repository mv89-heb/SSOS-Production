          ...
          {loading ? <Loading /> : users.map((user) => (
            <div key={user.id} className="flex items-center justify-between gap-4 border-b border-slate-100 py-4 last:border-0 dark:border-slate-800">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                {user.active && (
                  <button disabled={busy === `u-deactivate-${user.id}`} onClick={() => void run(`u-deactivate-${user.id}`, () => adminService.deactivateUser(user.id).then(() => undefined))} className="rounded-lg p-2 text-amber-600 hover:bg-amber-50 disabled:opacity-40 dark:hover:bg-amber-950/30" title="השבת משתמש">
                    <Ban className="h-4 w-4" />
                  </button>
                )}
                {!user.active && (
                  <button disabled={busy === `u-delete-${user.id}`} onClick={() => { if (window.confirm("למחוק את המשתמש לצמיתות? פעולה זו אינה הפיכה.")) void run(`u-delete-${user.id}`, () => adminService.deleteUser(user.id)); }} className="rounded-lg p-2 text-red-600 hover:bg-red-50 disabled:opacity-40 dark:hover:bg-red-950/30" title="מחיקה לצמיתות">
        ...
        <Panel title="ספקים" icon={<Building2 className="h-5 w-5" />}>
          {loading ? <Loading /> : suppliers.map((supplier) => (
            <div key={supplier.id} className="flex items-center justify-between gap-4 border-b border-slate-100 py-4 last:border-0 dark:border-slate-800">
              <div className="min-w-0">
                <div className="truncate font-bold text-slate-900 dark:text-white">{supplier.name}</div>
                <div className="text-xs text-slate-400">מזהה #{supplier.id}</div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${supplier.active ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" : "bg-slate-100 text-slate-500 dark:bg-slate-800"}`}>
                  {supplier.active ? "פעיל" : "מושבת"}
                </span>
                {supplier.active && (
                  <button disabled={busy === `s-deactivate-${supplier.id}`} onClick={() => void run(`s-deactivate-${supplier.id}`, () => adminService.deactivateSupplier(supplier.id).then(() => undefined))} className="rounded-lg p-2 text-amber-600 hover:bg-amber-50 disabled:opacity-40 dark:hover:bg-amber-950/30" title="השבת ספק">
                    <Ban className="h-4 w-4" />
                  </button>
                )}
                {!supplier.active && (
                  <button disabled={busy === `s-delete-${supplier.id}`} onClick={() => { if (window.confirm("למחוק את הספק לצמיתות? אם קיימים נתוני קטלוג, הפעולה תידחה.")) void run(`s-delete-${supplier.id}`, () => adminService.deleteSupplier(supplier.id)); }} className="rounded-lg p-2 text-red-600 hover:bg-red-50 disabled:opacity-40 dark:hover:bg-red-950/30" title="מחיקה לצמיתות">
                    <Trash2 className="h-4 w-4" />
                  </button>
                )}
              </div>

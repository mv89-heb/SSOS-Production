...
export function useAutoClassifyProducts() { const q = useQueryClient(); return useMutation({ mutationFn: (limit = 1000) => catalogService.autoClassifyProducts(limit), onSuccess: () => q.invalidateQueries({ queryKey: PRODUCTS_KEY }) }); }
...
export function useUpdateOffer(productId: number) { const q = useQueryClient(); return useMutation({ mutationFn: ({ offerId, input }: { offerId: number; input: UpdateOfferInput }) => catalogService.updateOffer(productId, input), onSuccess: () => q.invalidateQueries({ queryKey: offersKey(productId) }) }); }

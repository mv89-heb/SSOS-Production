    ...
    <>
      ...
      {mobileOpen && <div className="fixed inset-0 z-[90] md:hidden" role="dialog" aria-modal="true" aria-label="תפריט ניווט">
        ...
        <aside className="absolute inset-y-0 right-0 flex w-[min(86vw,340px)] flex-col bg-white shadow-2xl dark:bg-slate-950" dir="rtl">
          <div className="flex min-h-20 items-center justify-between border-b border-slate-100 px-4 dark:border-slate-800"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-700 to-violet-700 text-sm font-black text-white">ר</div><div><p className="text-sm font-black text-slate-900 dark:text-white">ישיבת אוהבי ירושלים</p><p className="text-xs font-bold text-indigo-600 dark:text-indigo-300">ראשית</p></div></div><button type="button" onClick={() => setMobileOpen(false)} className="rounded-xl p-2 text-slate-400 transition hover:bg-slate-100 dark:hover:bg-slate-900" aria-label="סגירת תפריט"><X className="h-5 w-5" /></button></div>
          ...
          <div className="border-t border-slate-100 p-4 dark:border-slate-800"><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-xs font-bold text-slate-700 dark:text-slate-200">{ORGANIZATION_NAME}</p><p className="mt-0.5 text-[11px] text-slate-400">{user?.full_name ?? "משתמש"}</p></div></div>
        </aside>
      </div>}
    </>
  );

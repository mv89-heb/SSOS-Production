...
export default function Sidebar() {
  ...
  const navigation = [
    ...
    { name: t("catalog"), href: "/dashboard/catalog", icon: Package },
    { name: t("orders"), href: "/dashboard/orders", icon: ShoppingCart },
    { name: t("audit"), href: "/dashboard/audit", icon: History },
  ...
  return (
    <aside className="hidden md:flex w-64 flex-col bg-white dark:bg-slate-950 border-e border-slate-200 dark:border-slate-800 h-screen sticky top-0">
      ...
      <nav className="flex-1 space-y-1 px-4 py-4 overflow-y-auto">
        {navigation.map((item) => {
          // "/dashboard" מקבל התאמה מדויקת בלבד, כדי שלא יודגש בכל תת-עמוד
          const isActive =
            pathname === item.href ||

...
import { LayoutDashboard, Users, Package, ShoppingCart, Settings, History, ShieldCheck, ChevronRight } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside dir="rtl" className="sticky right-0 top-0 z-30 hidden h-screen w-64 shrink-0 flex-col border-s border-slate-200/80 bg-white/95 backdrop-blur-md dark:border-slate-800 dark:bg-slate-950/95" aria-label="תפריט ניווט ראשי">
      <div className="flex min-h-24 items-center gap-3 border-b border-slate-100 px-5 dark:border-slate-800">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-700 to-violet-700 text-lg font-black text-white shadow-lg shadow-indigo-700/20">ר</div>
        <div className="min-w-0">
          <div className="truncate text-[15px] font-black leading-5 text-slate-950 dark:text-white">ישיבת אוהבי ירושלים</div>
          <div className="truncate text-xs font-bold text-indigo-600 dark:text-indigo-300">ראשית</div>
        </div>
      </div>
      ...
      <div className="border-t border-slate-100 p-4 dark:border-slate-800"><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-xs font-bold text-slate-700 dark:text-slate-200">{ORGANIZATION_NAME}</p><p className="mt-0.5 text-[11px] leading-4 text-slate-400">מערכת ניהול רכש</p></div></div>
    </aside>
  );
}

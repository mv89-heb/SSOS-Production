...
import { Search, X, LayoutDashboard, Package, ShoppingCart, Users, History, Settings, Upload } from "lucide-react";
import { cn } from "@/lib/utils";

const ACTIONS = [
  { label: "לוח בקרה", href: "/dashboard", icon: LayoutDashboard, keywords: "dashboard בית ראשי" },
  { label: "קטלוג מוצרים", href: "/dashboard/catalog", icon: Package, keywords: "מוצר קטלוג inventory ייבוא מחירון import" },
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center bg-slate-950/45 p-4 pt-[12vh] backdrop-blur-sm" onMouseDown={() => setOpen(false)}>
          <div role="dialog" aria-modal="true" aria-label="חיפוש מהיר" className="w-full max-w-xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900" onMouseDown={(event) => event.stopPropagation()}>
            <div className="flex items-center gap-3 border-b border-slate-100 px-4 dark:border-slate-800">
              <Search className="h-5 w-5 text-slate-400" />
              <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={handleKeyDown} placeholder="חפש מוצר, ספק, הזמנה או פעולה..." className="h-14 flex-1 bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400 dark:text-white" />
              <button type="button" onClick={() => setOpen(false)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800" aria-label="סגירה"><X className="h-4 w-4" /></button>
            </div>
            <div className="max-h-[55vh] overflow-y-auto p-2">
              {filtered.length === 0 ? (
                ...
                filtered.map((item, index) => {
                  ...
                  return (
                    <button key={item.href} type="button" onMouseEnter={() => setSelected(index)} onClick={() => navigate(item.href)} className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-3 text-right transition", index === selected ? "bg-indigo-50 text-indigo-900 dark:bg-indigo-950/60 dark:text-indigo-200" : "text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800")}>
                      <span className={cn("flex h-9 w-9 items-center justify-center rounded-lg", index === selected ? "bg-indigo-100 text-indigo-600 dark:bg-indigo-900/70 dark:text-indigo-300" : "bg-slate-100 text-slate-500 dark:bg-slate-800")}><Icon className="h-4 w-4" /></span>
            ...
            <div className="flex items-center gap-4 border-t border-slate-100 px-4 py-2.5 text-[10px] text-slate-400 dark:border-slate-800"><span>↑↓ ניווט</span><span>Enter פתיחה</span><span>Esc סגירה</span></div>
          </div>
        </div>
      )}
    </>

...
const CATALOG_KEYS = ["gf_supplier", "gf_product_status", "gf_stock", "gf_price_min", "gf_price_max", "gf_unit", "gf_missing", "gf_category"];
const SUPPLIER_KEYS = ["gf_supplier_status", "gf_contact", "gf_phone", "gf_email", "gf_days"];
  ...
  return (
    <section className="mb-5 rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900" aria-label="סינון ספקים">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        <div className="flex items-center gap-2 text-sm font-extrabold text-slate-800 dark:text-slate-100"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600 dark:bg-indigo-950/50 dark:text-indigo-300"><Filter size={16} /></span>סינון ספקים{activeCount > 0 && <span className="rounded-full bg-indigo-600 px-2 py-0.5 text-[11px] font-bold text-white">{activeCount}</span>}</div>
        <div className="flex flex-1 flex-wrap gap-2">
          <FilterSelect value={searchParams.get("gf_supplier_status") ?? "all"} onChange={(value) => setFilter("gf_supplier_status", value)}><option value="all">כל הסטטוסים</option><option value="active">פעילים בלבד</option><option value="inactive">לא פעילים</option></FilterSelect>
    ...
    <section className="mb-5 rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900" aria-label="סינון מתקדם לקטלוג">
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 text-sm font-extrabold text-slate-800 dark:text-slate-100"><span className="rounded-lg bg-indigo-50 p-2 text-indigo-600 dark:bg-indigo-950/50 dark:text-indigo-300"><SlidersHorizontal size={16} /></span>סינון מתקדם בקטלוג{activeCount > 0 && <span className="rounded-full bg-indigo-600 px-2 py-0.5 text-[11px] font-bold text-white">{activeCount} פעילים</span>}</div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <FilterSelect value={searchParams.get("gf_product_status") ?? "all"} onChange={(value) => setFilter("gf_product_status", value)}><option value="all">כל הסטטוסים</option><option value="active">פעילים בלבד</option><option value="inactive">לא פעילים</option></FilterSelect>

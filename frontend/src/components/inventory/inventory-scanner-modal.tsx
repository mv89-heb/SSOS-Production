...
const movementMeta: Record<"count" | "receipt" | "issue", { label: string; description: string }> = {
  count: { label: "ספירה", description: "עדכון מספר הארגזים שנמצאו במחסן" },
  receipt: { label: "קבלה", description: "הוספת ארגזים למלאי" },
  issue: { label: "ניפוק", description: "הפחתת ארגזים מהמלאי" },
};

const stockUnit = (product: Product) => product.unit === "יחידה" ? "יחידות" : "ארגזים";
...
export default function InventoryScannerModal({ open, onClose }: InventoryScannerModalProps) {
  ...
  const saveMovement = async () => {
    if (!product || !movementType || saving) return;
    const qty = Number(quantity);
    if (!Number.isInteger(qty) || qty < 0 || (movementType !== "count" && qty <= 0)) {
      setError(movementType === "count" ? `בספירה יש להזין מספר שלם שאינו שלילי (${stockUnit(product)}).` : `בקבלה או ניפוק יש להזין כמות שלמה גדולה מאפס (${stockUnit(product)}).`);
      return;
    }
    ...
    try {
      ...
      setSuccess(`${label} של ${product.name} נקלטה בהצלחה. מלאי נוכחי: ${balanceAfter} ${stockUnit(product)}.`);
      setMovementType(null);
      setQuantity("");
      setNote("");
      // A successful scan action is a completed workflow. Close the modal so
      // the operator can immediately continue working on the inventory screen.
      onClose();
    } catch (e) {
      ...
      <div className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl dark:bg-slate-950 sm:rounded-3xl">
        ...
        <div className="p-4 sm:p-5">
          ...
          {product && <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950/20">
            <div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-emerald-600" size={24} /><div className="min-w-0 flex-1"><h3 className="text-xl font-black">{product.name}</h3><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">מק״ט: {product.sku || "—"} · ברקוד: {product.barcode || "—"}</p><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-white p-3 dark:bg-slate-950"><span className="block text-xs text-slate-500">מלאי ({stockUnit(product)})</span><strong className="text-xl">{product.current_stock ?? 0}</strong></div><div className="rounded-xl bg-white p-3 dark:bg-slate-950"><span className="block text-xs text-slate-500">מינימום ({stockUnit(product)})</span><strong className="text-xl">{product.min_stock ?? "—"}</strong></div><div className="rounded-xl bg-white p-3 dark:bg-slate-950"><span className="block text-xs text-slate-500">יעד ({stockUnit(product)})</span><strong className="text-xl">{product.recommended_stock ?? "—"}</strong></div></div></div></div>

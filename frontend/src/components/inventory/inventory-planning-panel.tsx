...
import { CalendarDays, CheckCircle2, Clock3, RefreshCw, Sparkles, TriangleAlert, Truck } from "lucide-react";
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
      <div className="border-b border-slate-200 bg-gradient-to-l from-indigo-50 to-white p-5 dark:border-slate-800 dark:from-indigo-950/30 dark:to-slate-950">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-sm font-black text-indigo-700 dark:text-indigo-300"><CalendarDays size={19}/> ספירת מלאי ותכנון תקופות</div>
            <h2 className="mt-1 text-xl font-black">המלאי נקבע לפי מה שנמצא בפועל</h2>
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">אין צורך לדווח על כל לקיחה מהמחסן. כל ספירה יוצרת נקודת אמת חדשה, והרכישות והספירות משמשות לחישוב קצב הצריכה.</p>
          </div>
          <button onClick={seedHolidays} disabled={busy} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 text-sm font-black text-white disabled:opacity-50"><Sparkles size={17}/>{busy ? "טוען…" : "הוסף תקופות חגים"}</button>
        </div>
      </div>
      <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className={`rounded-2xl border p-4 ${status?.completed ? "border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/20" : "border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/20"}`}>
          <div className="flex items-center gap-2 text-xs font-black text-slate-500"><CheckCircle2 size={16}/> השלמת הספירה</div>
          <div className="mt-2 text-2xl font-black">{status?.completion_percent ?? 0}%</div>
          <div className="mt-1 text-xs text-slate-500">{status?.counted_products_last_7_days ?? 0} מתוך {status?.active_products ?? 0} מוצרים</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-2 text-xs font-black text-slate-500"><Clock3 size={16}/> הספירה הבאה</div>
          <div className="mt-2 text-lg font-black">{fmtDate(status?.next_due_date)}</div>
          <div className="mt-1 text-xs text-slate-500">ברירת מחדל: יום ראשון בשעה {status?.count_time ?? "08:00"}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-2 text-xs font-black text-slate-500"><TriangleAlert size={16}/> מצב</div>
          <div className="mt-2 text-lg font-black">{status?.due ? "הספירה דורשת טיפול" : status?.completed ? "הספירה השבועית הושלמה" : "מתכוננים לספירה"}</div>
          <div className="mt-1 text-xs text-slate-500">המערכת אינה מאפסת את המלאי; היא שומרת Snapshot חדש.</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-2 text-xs font-black text-slate-500"><Truck size={16}/> חגים פעילים</div>
          <div className="mt-2 text-lg font-black">{upcoming.length} תקופות</div>
          <div className="mt-1 text-xs text-slate-500">המכפיל וימי ההזמנה ניתנים להתאמה.</div>
        </div>
      </div>

...
def upgrade():
    ...
    indexes = {idx["name"] for idx in sa.inspect(bind).get_indexes("price_observations")}
    for name, columns in {"ix_price_observations_tenant_id":["tenant_id"],"ix_price_observations_product_id":["product_id"],"ix_price_observations_supplier_id":["supplier_id"],"ix_price_observation_tenant_product":["tenant_id","product_id"],"ix_price_observation_tenant_supplier":["tenant_id","supplier_id"],"ix_price_observation_document":["tenant_id","source_document_id"],"ix_price_observation_observed_at":["tenant_id","observed_at"]}.items():
        if name not in indexes: op.create_index(name, "price_observations", columns)
def downgrade():
    bind = op.get_bind(); inspector = sa.inspect(bind)
    if "price_observations" not in inspector.get_table_names(): return
    indexes = {idx["name"] for idx in inspector.get_indexes("price_observations")}
    for name in ("ix_price_observation_observed_at","ix_price_observation_document","ix_price_observation_tenant_supplier","ix_price_observation_tenant_product","ix_price_observations_supplier_id","ix_price_observations_product_id","ix_price_observations_tenant_id"):
        if name in indexes: op.drop_index(name, table_name="price_observations")

...
def _setup(db, tenant_id):
    ...
    db.session.flush()
    product = Product(tenant_id=tenant_id, supplier_id=primary.id, name="Milk", sku="MILK-OFFER",
                      current_price=12, currency="ILS", unit="יחידה", active=True)
    db.session.add(product)
    db.session.flush()
    offer = SupplierProductOffer(tenant_id=tenant_id, product_id=product.id, supplier_id=alternate.id,
                                 price=8, currency="ILS", unit="יחידה", active=True)
...
def test_selected_supplier_offer_is_snapshotted_into_order(db, tenant_a_admin):
    ...
    order = OrderService(tenant_id).create_order(user, {
...
def test_order_rejects_offer_from_wrong_supplier(db, tenant_a_admin):
    tenant_id = tenant_a_admin[0]["tenant"]["id"]
    user = db.session.query(User).filter(User.email == "admin@acme.test").one()
    primary, _, product, offer = _setup(db, tenant_id)
    with pytest.raises(Conflict, match="does not match"):
        OrderService(tenant_id).create_order(user, {
            "supplier_id": primary.id,
            "items": [{"product_id": product.id, "supplier_offer_id": offer.id, "quantity": 1}],
        })


def test_order_falls_back_to_primary_supplier_price_without_offer(db, tenant_a_admin):
    tenant_id = tenant_a_admin[0]["tenant"]["id"]
    user = db.session.query(User).filter(User.email == "admin@acme.test").one()
    primary = Supplier(tenant_id=tenant_id, name="Primary Only")
    db.session.add(primary)
    db.session.flush()
    product = Product(tenant_id=tenant_id, supplier_id=primary.id, name="Bread", sku="BREAD-PRIMARY",
                      current_price=5.5, currency="ILS", active=True)
    db.session.add(product)
    db.session.commit()
    order = OrderService(tenant_id).create_order(user, {

...
3. Set `DATABASE_URL` to your Neon connection string in the Render dashboard
   (marked `sync: false` in `render.yaml`, so it isn't stored in git).
4. Deploy. Check `GET /health` and `GET /health/ready` once live.

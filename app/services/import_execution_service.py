        ...
        try:
            for row in importable_rows:
                if not row.supplier_name and not row.matched_supplier_id:
                    raise ImportExecutionError(f"Row {row.row_number}: no supplier could be determined.")
                ...
                for offer in row.offers or []:
                    ...
                    if offer_price is None:
                        raise ImportExecutionError(f"Row {row.row_number}: supplier offer is missing a price.")

                    is_primary = bool(offer.get("is_primary")) or (
                    ...
                    offer_supplier_id = offer.get("matched_supplier_id") or resolve_or_create_supplier(supplier_name)
                    try:
            ...
            AuditService.log_event(
                ...
                f"{len(skipped_rows)} row(s) skipped",
                {
                    "import_session_id": session_id,

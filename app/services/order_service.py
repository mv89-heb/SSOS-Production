...
class OrderService:
    ...
    def list_orders(self, status: str = None, limit: int = 50, offset: int = 0) -> list:
        ...
        if status:
            if status not in VALID_STATUSES: raise BadRequest(f"Invalid status filter: {status}")
    ...
    def update_order(self, user, order_id: int, payload: dict) -> Order:
        ...
        if "items" in payload:
            ...
            enriched_items = self._build_snapshot_items(input_items); totals = SnapshotService.compute_totals(enriched_items)
            order.items = enriched_items; order.subtotal = totals["subtotal"]; order.discount_total = totals["discount_total"]; order.tax_total = totals["tax_total"]; order.final_total = totals["final_total"]
    ...
    def submit_order(self, user, order_id: int) -> Order:
        order = self.repo.get_by_id_for_update(order_id)
        if order is None: raise NotFound("Order not found")
        if order.status != STATUS_DRAFT: raise Conflict(f"Only '{STATUS_DRAFT}' orders can be submitted (current: '{order.status}')")
        if user.role not in (ROLE_MANAGER, ROLE_ADMIN) and order.user_id != user.id: raise Conflict("Only the order creator or a manager can submit this draft")
        if not order.items: raise Conflict("An order must contain at least one item before submission")
        order.status = STATUS_SUBMITTED; SnapshotService.apply_snapshot(order)
    ...
    def approve_order(self, user, order_id: int) -> Order:
        order = self.repo.get_by_id_for_update(order_id)
        if order is None: raise NotFound("Order not found")
        if order.status != STATUS_SUBMITTED: raise Conflict(f"Only '{STATUS_SUBMITTED}' orders can be approved (current: '{order.status}')")
        if order.user_id == user.id: raise Conflict("The order creator cannot approve their own order")
        order.status = STATUS_APPROVED; AuditService.log_event(self.tenant_id, user.id, "order.approved", f"Order {order.order_number} approved", {"order_id": order.id, "approved_by": user.id}); return order

    def reject_order(self, user, order_id: int, reason: str = "") -> Order:
        order = self.repo.get_by_id_for_update(order_id)
        if order is None: raise NotFound("Order not found")
        if order.status != STATUS_SUBMITTED: raise Conflict(f"Only '{STATUS_SUBMITTED}' orders can be rejected (current: '{order.status}')")
        if order.user_id == user.id: raise Conflict("The order creator cannot reject their own order")
    ...
    def _build_snapshot_items(self, input_items: list) -> list:
        ...
        for item in input_items:
            ...
            if not isinstance(product_id, int) or isinstance(product_id, bool) or product_id <= 0: raise BadRequest("Each order item requires a valid product_id")
            product_ids.append(product_id)
        products = {p.id: p for p in self.product_repo.get_many_by_ids(product_ids)}
        enriched_items = []
        for item in input_items:
            p_id = item["product_id"]; p = products.get(p_id)
            if not p: raise NotFound(f"Product {p_id} not found in your catalog")
            if not p.active: raise Conflict(f"Product {p_id} is inactive and cannot be ordered")
            raw_qty = item.get("quantity", 1)
            if isinstance(raw_qty, bool): raise BadRequest("Quantity must be a positive integer")
            try: qty = int(raw_qty)

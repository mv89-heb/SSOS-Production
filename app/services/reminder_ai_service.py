...
class ReminderAIService:
    ...
    def _text_blob(order, user_text: str = "") -> str:
        ...
        item_text = " ".join(
            str(item.get("description") or item.get("name") or item.get("product_name") or "")
        ...
        return " ".join([
            ...
            str(user_text or ""),
            item_text,
            str(order.supplier_name or ""),
        ]).casefold()

    @classmethod
    def deterministic_score(cls, order, now: datetime, user_text: str = "", deadline: datetime | None = None) -> int:
        """Calculate a conservative baseline before asking Gemini."""
        text = cls._text_blob(order, user_text)
        score = 0
        urgent_terms = ("דחוף", "קריטי", "חייב", "בהקדם", "היום", "תקוע", "עוצר", "urgent", "critical", "asap")
        if any(term in text for term in urgent_terms):
            score += 30
        blocking_terms = ("עוצר עבודה", "לא יכול לעבוד", "מעכב", "תקלה", "עצירה", "blocked", "blocking")
    ...
    @classmethod
    def _call_gemini(cls, payload: dict) -> dict | None:
        if not cls.enabled():
            return None
        try:
    ...
    def analyze(cls, order, now: datetime | None = None, user_text: str = "", deadline: datetime | None = None) -> dict:
        ...
        result = cls._call_gemini(payload)
        ...
        if isinstance(result, dict) and result.get("urgency") == "critical" and score >= 80:
            urgency = "critical"
        ...
        return {
            ...
            "escalate_after_occurrences": escalate,
            "source": "gemini+rules" if result else "rules",
    ...
    def choose_candidate(cls, order, candidates: list[dict], now: datetime) -> dict | None:
        ...
        try:
            ...
            current_app.logger.exception("Gemini candidate selection failed")
            return None

    @classmethod
    def parse_natural_language(cls, order, text: str, now: datetime | None = None) -> dict:
        """Turn Hebrew natural language into a bounded reminder plan."""
        now = now or datetime.now(timezone.utc)
        text = (text or "").strip()
        if not text:
        ...
        payload = {
            "order": {"order_number": order.order_number, "supplier_name": order.supplier_name, "status": order.status, "notes": order.notes},
            "current_time": now.isoformat(),
            "timezone": current_app.config.get("GOOGLE_CALENDAR_TIMEZONE", "Asia/Jerusalem"),
            "request": text,
        }
        result = cls._call_gemini({**payload, "task": "Extract reminder intent. Return only the requested delay and recurrence; never invent a calendar date."})
        if result:
            first = max(5, min(10080, int(result.get("first_reminder_minutes") or cls.DEFAULT_REPEAT_MINUTES)))
            repeat = max(5, min(10080, int(result.get("repeat_minutes") or cls.DEFAULT_REPEAT_MINUTES)))
            analysis = cls.analyze(order, now=now, user_text=text)
            return {
                **analysis,
                "first_reminder_at": (now + timedelta(minutes=first)).isoformat(),
                "repeat_minutes": repeat,
                "escalate_after_occurrences": int(result.get("escalate_after_occurrences") or analysis["escalate_after_occurrences"]),
                "interpretation": text,
            }

        # Safe deterministic fallback for common Hebrew phrases.
        lower = text.casefold()
        minutes = cls.DEFAULT_REPEAT_MINUTES
        if "עוד שעה" in lower or "שעה" in lower:
            minutes = 60
        elif "חצי שעה" in lower:
            minutes = 30
        elif "שעתיים" in lower or "שעתיים" in lower:
            minutes = 120
        elif "מחר" in lower:
            minutes = 24 * 60
        analysis = cls.analyze(order, now=now, user_text=text)
        return {**analysis, "first_reminder_at": (now + timedelta(minutes=minutes)).isoformat(), "interpretation": text}
    ...
    def apply_analysis(cls, order, analysis: dict, *, reminder_at: datetime | None = None, mode: str = "ai") -> None:
        ...
        snapshot.update({
            ...
            "ai": analysis,
            "recurrence": {
                "every_minutes": int(analysis["repeat_minutes"]),
                "max_occurrences": 0,
            },
            "escalation": {
                "after_occurrences": int(analysis["escalate_after_occurrences"]),
            },
            "occurrences": 0,
            "escalated": False,
    ...
    def urgency_display(cls, urgency: str) -> dict:
        return {
            ...
            "normal": {"label": "רגילה", "emoji": "🟡"},
            "high": {"label": "גבוהה", "emoji": "🟠"},
            "critical": {"label": "קריטית", "emoji": "🔴"},
        }.get(urgency, {"label": "רגילה", "emoji": "🟡"})

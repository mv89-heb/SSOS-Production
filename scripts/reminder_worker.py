...
def _notify(user_id, tenant_id, title, message, action_url, notification_type, order_id, order_number):
    notification = Notification(tenant_id=tenant_id, user_id=user_id, title=title, message=message, status=STATUS_UNREAD, action_url=action_url, notification_type=notification_type)
    db.session.add(notification)
    db.session.flush()
    AuditService.log_event(tenant_id, user_id, notification_type, title=title, metadata={"order_id": order_id, "order_number": order_number})
    db.session.commit()
    send_to_user(user_id, {"title": title, "body": message, "url": action_url, "notification_id": notification.id, "order_id": order_id, "type": notification_type})
...
def process_due_reminders() -> int:
    ...
    due_orders = list(db.session.execute(select(Order).where(Order.next_reminder_at.is_not(None), Order.next_reminder_at <= now, Order.reminder_state == REMINDER_PENDING, Order.status.notin_((STATUS_SENT, STATUS_COMPLETED, STATUS_CANCELLED))).with_for_update(skip_locked=True)).scalars().all())
    ...
    for order in due_orders:
        ...
        notification = Notification(tenant_id=order.tenant_id, user_id=order.user_id, title=title, message=message, status=STATUS_UNREAD, action_url=action_url, notification_type=notification_type)
        db.session.add(notification)
        db.session.flush()
        AuditService.log_event(order.tenant_id, order.user_id, notification_type, title=title, metadata={"order_id": order.id, "order_number": order.order_number, "occurrence": occurrences, "recurring": should_repeat, "urgency": (rules.get("ai") or {}).get("urgency"), "urgency_score": (rules.get("ai") or {}).get("score")})
        send_to_user(order.user_id, {"title": title, "body": message, "url": action_url, "notification_id": notification.id, "order_id": order.id, "type": notification_type})
        ...
        if escalation_after and occurrences >= escalation_after and not rules.get("escalated"):
            managers = db.session.execute(select(User).where(User.tenant_id == order.tenant_id, User.active.is_(True), User.role == ROLE_MANAGER)).scalars().all()
            for manager in managers:
                manager_message = f"הזמנה {order.order_number} עדיין דורשת טיפול של {order.supplier_name}. {urgency_label}."
                _notify(manager.id, order.tenant_id, "התראת הסלמה על תזכורת", manager_message, action_url, "reminder_escalation", order.id, order.order_number)
